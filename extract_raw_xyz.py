...
for f in files:
    try:
        ...
        mask = (df["Start Time"] >= "2026-04-16 10:00:00") & (df["Start Time"] < "2026-04-16 10:00:02.5")
        sub = df.loc[mask]
        if len(sub) > 0:
            dfs.append(sub)
    except:
...
with open(os.path.join(root, "raw_xyz_samples.json"), "w", encoding="utf-8") as f:
    json.dump(result, f, ensure_ascii=False, indent=2)
print(f"OK: {len(result['10 Hz'])} rows per band")
